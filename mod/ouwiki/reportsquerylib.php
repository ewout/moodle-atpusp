 ...
 * @param int $roleid Role ID or 0 for any
 * @return string Comma-separated list of IDs or false if no users
 */
function ouwiki_get_usersin($contexts,$groupid=0,$roleid=0) {
    global $CFG;
...
");
        ...
        $rolecondition=$roleid ? "AND ra.roleid=$roleid" : '';
        $rs=get_recordset_sql("
SELECT
	  DISTINCT u.id AS userid
FROM 
 ...
 * @param unknown_type $subwikiid  the subwiki
 * @param unknown_type $groupid    the user group
 * @param unknown_type $roleid     the user role, 0 for all
 * @return user id and name, edit count, number of edited pages, first and last days of editing
 */
function ouwiki_get_usersedits($contexts,$subwikiid, $groupid=0, $roleid=0) {
    global $CFG;
    
    $userlist=ouwiki_get_usersin($contexts,$groupid,$roleid);
    if(!$userlist) {
        return array();
    }    
...
function ouwiki_get_userscomments($contexts, $subwikiid, $groupid=0, $roleid=0) {
    $userlist=ouwiki_get_usersin($contexts,$groupid,$roleid);
    if(!$userlist) {
        return array();
    }    
