...
function ouwiki_set_extra_subwiki_fields(&$subwiki,$ouwiki,$context,$othergroup=false,$defaultwiki=false) {
    ...
    if($othergroup) {
        $subwiki->canedit &= has_capability('mod/ouwiki:editallsubwikis',$context);
    }
    // Editing might be turned off for the wiki at the moment
    $subwiki->canedit &= is_null($ouwiki->editbegin) || time() >= $ouwiki->editbegin;
    $subwiki->canedit &= is_null($ouwiki->editend) || time() < $ouwiki->editend;
 ...
 */
function ouwiki_display_page($subwiki,$cm,$pageversion,$gewgaws=false) {
    // Get comments
    if($gewgaws) {
    ...
    $result.='<div class="ouw_heading"><h1 id="ouw_topheading">'.$title.'</h1>'.
        ($gewgaws
            ? ouwiki_internal_display_heading_bit(1,$pageversion->title,$subwiki,$cm,$comments,null,$returncomments)
            : '</div>');
    ...
    if($gewgaws && $pageversion->recentversions) {
        ...
        $result.='; <a href="history.php?'.$params.'">'.get_string('seedetails','ouwiki').'</a></span></div>';
    }
...
function ouwiki_internal_re_heading_bits($matches) {
    ...
    return '<div class="ouw_heading ouw_heading'.$matches[1].'"><h'.$matches[1].' id="ouw_s'.$matches[2].'">'.$matches[3].'</h'.$matches[1].'>'.
        ouwiki_internal_display_heading_bit($matches[1],
            $ouwiki_internal_re->pagename,$ouwiki_internal_re->subwiki,$ouwiki_internal_re->cm,$ouwiki_internal_re->comments,$matches[2]);
}
...
function ouwiki_internal_display_heading_bit($headingnumber,$pagename,$subwiki,$cm,&$comments,$xhtmlid,&$returncomments=false) {
    ...
    // Addition to offer Save To Portfolio ability for postings
     global $CFG;
        ...
        if (has_capability('mod/portfolio:doanything', $modcontext, NULL, true, 'portfolio:doanything:false', 'portfolio')) {
            ...
            $result.='<div style="display: inline; float: right;"><a href="'.$CFG->wwwroot.'/mod/portfolio/moodle/ouwiki/saveToPortfolio.php?'.$params.($xhtmlid ? '&amp;section='.$xhtmlid : '').'" title="'.$wikiposttoportfolio.'"><span class="accesshide">'.$wikiposttoportfolio.'</span><img src="'.$CFG->wwwroot.'/mod/portfolio/html/_design/icon_download.gif" alt="" style="width: 11px; height: 11px;"/></a></div>';
...
function ouwiki_print_tabs($selected,$pagename,$subwiki,$cm,$context,$pageexists=true) {
    ...
    $tabs=array();
    $tabs[]=$tabrow;
    print_tabs($tabs,$selected,$pageexists?'':array('edit'));
...
function ouwiki_print_start($ouwiki,$cm,$course,$subwiki,$pagename,$context,$afterpage=null,$hideindex=false,$notabs=false,
    ...
    if(!$hideindex) {
        ...
        if($isindex) {
            print '<li><span>'.get_string('index','ouwiki').'</span></li>';
        } else {
            print '<li><a href="wikiindex.php?'.
        ...
        if($ishistory) {
            print '<li><span>'.get_string('wikirecentchanges','ouwiki').'</span></li>';
        } else {
            print '<li><a href="wikihistory.php?'.
        ...
        if($isreports) {
            print '<li><span>'.get_string('reports','ouwiki').'</span></li>';
        } else if(has_capability('mod/ouwiki:viewcontributions',$context)) {
            print '<li><a href="reportssummary.php?'.
        ...
        if(ouwiki_search_installed()) {
            ...
            print '<input type="submit" value="'.get_string('search','ouwiki').'" /></form>';
        }
...
*/
function ouwiki_create_new_section($course,$cm,$ouwki,$subwiki,$pagename,$newcontent, $sectionheader){

    $sourcepage=ouwiki_get_current_page($subwiki, $pagename);
    $sectiondetails = ouwiki_get_new_section_details($sourcepage->xhtml, $sectionheader);
    ouwiki_save_new_version_section($course,$cm,$ouiwki, $subwiki,$pagename,$sourcepage->xhtml, $newcontent,$sectiondetails);
