...
$chapter = required_param('chapter', PARAM_INT);
$course = required_param('course', PARAM_INT);
$presenter = required_param('presenter', PARAM_INT);

$volume = optional_param('volume', NULL, PARAM_CLEAN);

if ($volume) {
    $SESSION->playerVolume = $volume;
}
	
$arr = new stdClass();
$arr->course_id = $course;
$arr->username = $user;
$arr->chapter_id = $chapter;
$arr->presenter_id = $presenter;

if (!chapter_completed($chapter, $username)) {
    insert_record('presenter_chapters_users', $arr);
}
