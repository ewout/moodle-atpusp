...
*/
	...
	if (isset($_POST['unzip'])) {
		redirect("zip.php?id={$id}&amp;groupid={$groupid}&amp;rootdir={$rootdir}&amp;zipid={$linkid}&amp;action=unzip");
	}
	if (isset($_POST['add'])) {
		if ($linkname == '' || $linkurl == '') {	// verify's input
			...
			if ($store->radioval == 'file') {
				...
				if ($_FILES['linkfile']['size'] > ($adminsettings->maxupload * (1048576))) {
					error(get_string('errfiletoolarge', 'block_file_manager').'FileManage Settings max size: '.$adminsettings->maxupload);
				} else {
