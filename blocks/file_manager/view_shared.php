...
*/
	...
	if (isset($original)) {
		$courselink .= "<a href=\"view_shared.php?id={$id}&amp;ownertype={$ownertype}\">".get_string("othersharedfiles", 'block_file_manager')."</a> -> ".get_string("sharedfiles", 'block_file_manager');
		...
		$courselink .= get_string('othersharedfiles', 'block_file_manager');
	...
	if (!isset($catlinkid) && !isset($foldlinkid)) {
		print_header(strip_tags(get_string('othersharedfiles', 'block_file_manager')), "$course->fullname",
			" $courselink", '', '', true, '', navmenu($course), '', '');
	} else {
		print_header(strip_tags(get_string('othersharedfiles', 'block_file_manager')), "$course->fullname",
			'', '', '', true, '', '', '', '');
	}
