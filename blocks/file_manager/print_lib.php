...
/****************************************************************************/
echo'<LINK REL="stylesheet" HREF="file_manager.css" TYPE="text/css">';
...
// $id 		= course id
function fm_print_category_list($id, $rootdir, $groupid=0) {
	global $CFG, $USER;
...
function fm_print_user_files_form($id=1, $rootdir=0, $action='none', $groupid=0) {
	global $CFG, $USER;
	...
	if ($allfolders) {
		foreach($allfolders as $folder) {
			...
			$name = "<a href=\"{$CFG->wwwroot}/".fm_get_root_dir()."/view.php?id={$id}&groupid={$groupid}&rootdir={$folder->id}{$moveurl}\"><img border=\"0\" src=\"$CFG->wwwroot/blocks/file_manager/pix/folder.gif\" alt=\"".get_string('msgfolder','block_file_manager',$folder->name)."\" /> $folder->name</a>";
	...
	if (!$alllinks) {
		...
		foreach($alllinks as $link) {
			...
			if ($link->type == TYPE_FILE) {
				...
				$name = "<a target=\"urlpopup\" title=\"".get_string('msgopenfile', 'block_file_manager', $link->link)."\" href=\"$tmphref\" onClick=\"window.open('$tmphref','urlpopup','$options');\"><img src=\"$CFG->wwwroot/blocks/file_manager/pix/file.gif\" >&nbsp;$link->name</a>";
			} else if ($link->type == TYPE_URL) {
				$name = "<a target=\"urlpopup\" title=\"".get_string("msgopenlink",'block_file_manager')."\" href=\"$link->link\"".
				"onClick=\"window.open('$link->link','urlpopup','$options');\"><img src=\"$CFG->wwwroot/blocks/file_manager/pix/www.gif\">&nbsp;$link->name</a>";
				...
				$tmphref = "{$CFG->wwwroot}/blocks/file_manager/file.php?cid={$id}&groupid=$groupid&fileid={$link->id}";
				$name = "<a target=\"urlpopup\" title=\"".get_string('msgopenfile', 'block_file_manager', $link->link)."\" href=\"$tmphref\" onClick=\"window.open('$tmphref','urlpopup','$options');\"><img src=\"$CFG->wwwroot/blocks/file_manager/pix/zip.gif\" >&nbsp;$link->name</a>";                                                                                                                                        
			}
			/*
			...
			*/
			...
			$desc = shorten_text($link->description, 120);
...
function fm_print_share_course_members($id, $linkid=NULL, $type=0) {
	...
	if ($id == 1) {
		...
		$sqltoexecute = "SELECT {$CFG->prefix}user.firstname, {$CFG->prefix}user.lastname, {$CFG->prefix}user.id,  {$CFG->prefix}role.name FROM {$CFG->prefix}user, {$CFG->prefix}course_display, {$CFG->prefix}role_assignments, {$CFG->prefix}role  WHERE (course = $id AND {$CFG->prefix}user.id = {$CFG->prefix}role_assignments.userid AND {$CFG->prefix}role_assignments.roleid = {$CFG->prefix}role.id)";
	...
	if ($sharedfolders) {
		foreach($sharedfolders as $sf) {
			...
			$name = "<a target=\"foldpopup\" title=\"".get_string("msgopenlink",'block_file_manager')."\" href=\"$linkurl\"".
				"onClick=\"window.open('$linkurl','foldpopup','$options');\"><img src=\"$CFG->wwwroot/blocks/file_manager/pix/folder.gif\" >&nbsp;$sf->name</a>";
	...
	if ($sharedfiles) {
		foreach($sharedfiles as $sf) {
			...
			if ($sf->type == TYPE_FILE) {
				...
				$name = "<a target=\"urlpopup\" title=\"".get_string("msgopenfile",'block_file_manager', $sf->link)."\" href=\"$tmphref\" onClick=\"window.open('$tmphref','urlpopup','$options');\"><img src=\"$CFG->wwwroot/blocks/file_manager/pix/file.gif\" >&nbsp;$sf->name</a>";
			} else if ($sf->type == TYPE_URL) {
				$name = "<a target=\"urlpopup\" title=\"".get_string("msgopenlink",'block_file_manager')."\" href=\"$sf->link\"".
				"onClick=\"window.open('$sf->link','urlpopup','$options');\"><img src=\"$CFG->wwwroot/blocks/file_manager/pix/www.gif\" >&nbsp;$sf->name</a>";
			} else if ($sf->type == TYPE_ZIP) {
				$tmphref = "$CFG->wwwroot/blocks/file_manager/file.php?cid=$id&groupid=$groupid&fileid=$sf->id";
				$name = "<a target=\"urlpopup\" title=\"".get_string("msgopenfile",'block_file_manager', $sf->link)."\" href=\"$tmphref\" onClick=\"window.open('$tmphref','urlpopup','$options');\"><img src=\"$CFG->wwwroot/blocks/file_manager/pix/zip.gif\" >&nbsp;$sf->name</a>";
			}
			$desc = $sf->description;
			$cat = fm_get_user_categories($sf->category);
			$date = "<font size=1>".userdate($sf->timemodified, "%m/%d/%Y  %H:%I")."</font>";
	...
	if ($sharedfolder) {
		foreach($sharedfolder as $sf) {
			...
			$name = "<a target=\"foldpopup\" title=\"".get_string("msgopenfold",'block_file_manager')."\" href=\"$tmpurl\">".
					"<img src=\"$CFG->wwwroot/blocks/file_manager/pix/folder.gif\" >&nbsp;$sf->name</a>";
