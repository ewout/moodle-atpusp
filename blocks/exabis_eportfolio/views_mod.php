...
if ($action == 'userlist') {
	echo json_encode(exabis_eportfolio_get_shareable_courses_with_users('sharing'));
	exit;
}
...
class block_exabis_eportfolio_view_edit_form extends moodleform {
	function definition() {
		...
		$mform->addElement('textarea', 'description', get_string("title", "block_exabis_eportfolio"), 'cols="60" rows="5"');
...
echo '<div class="view-data view-group'.(!$view?' view-group-open':'').'">';
	...
	echo '<div class="view-group-body">';
		...
		echo '<fieldset class="clearfix"><legend class="ftoggler">'.get_string('viewinformation', 'block_exabis_eportfolio').'</legend>';
			echo '<div class="fitem required"><div class="fitemtitle"><label>'.get_string('viewtitle', 'block_exabis_eportfolio').'<img class="req" title="Required field" alt="Required field" src="'.$CFG->wwwroot.'/pix/req.gif" /> </label></div><div class="felement ftext">'.$form['elements_by_name']['name']['html'].'</div></div>';
			echo '<div class="fitem"><div class="fitemtitle"><label>'.get_string('viewdescription', 'block_exabis_eportfolio').'</label></div><div class="felement ftext">'.$form['elements_by_name']['description']['html'].'</div></div>';
		echo '</fieldset>';
		echo '</div>';
	echo '</div>';
echo '</div>';
