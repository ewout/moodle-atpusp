...
if ($items) {
	...
	foreach ($items as $item) {
		...
		$table->data[$item_i]['name'] = "<a href=\"".s("{$CFG->wwwroot}/blocks/exabis_eportfolio/shared_item.php?courseid=$courseid&access=portfolio/id/".$USER->id."&itemid=$item->id&backtype=".$type)."\">" . format_string($item->name) . "</a>";
		if ($item->intro) {
		...
		*/
		...
		if (block_exabis_eportfolio_feature_enabled('share_item')) {
		...
		if ($item->type == 'file' && block_exabis_eportfolio_feature_enabled('copy_to_course'))
			$icons .= '<a href="'.$CFG->wwwroot.'/blocks/exabis_eportfolio/copy_item_to_course.php?courseid='.$courseid.'&amp;itemid='.$item->id.'&backtype='.$type.'">'.get_string("copyitemtocourse", "block_exabis_eportfolio").'</a> ';
