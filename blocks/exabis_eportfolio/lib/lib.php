...
function block_exabis_eportfolio_feature_enabled($feature)
{
	global $CFG;
	if ($feature == 'views')
		return block_exabis_eportfolio_get_active_version() >= 3;
	if ($feature == 'share_item') 
		return block_exabis_eportfolio_get_active_version() < 3;
	if ($feature == 'copy_to_course') 
		return !empty($CFG->block_exabis_eportfolio_feature_copy_to_course);
	die('wrong feature');
}
...
function block_exabis_eportfolio_print_header($item_identifier, $sub_item_identifier = null) {
	...
	if (strpos($item_identifier, 'bookmarks') === 0) {
		...
		$tabs_sub['bookmarksall'] = new tabobject('bookmarksall', s($CFG->wwwroot.'/blocks/exabis_eportfolio/view_items.php?courseid='.$COURSE->id),
									get_string("bookmarksall","block_exabis_eportfolio"), '', true);
		$tabs_sub['bookmarkslinks'] = new tabobject('bookmarkslinks', s($CFG->wwwroot.'/blocks/exabis_eportfolio/view_items.php?courseid='.$COURSE->id.'&type=link'),
									get_string("bookmarkslinks","block_exabis_eportfolio"), '', true);
		$tabs_sub['bookmarksfiles'] = new tabobject('bookmarksfiles', s($CFG->wwwroot.'/blocks/exabis_eportfolio/view_items.php?courseid='.$COURSE->id.'&type=file'),
									get_string("bookmarksfiles","block_exabis_eportfolio"), '', true);
		$tabs_sub['bookmarksnotes'] = new tabobject('bookmarksnotes', s($CFG->wwwroot.'/blocks/exabis_eportfolio/view_items.php?courseid='.$COURSE->id.'&type=note'),
...
function block_exabis_eportfolio_parse_view_sort($sort, $for_shared = false)
{
...
function block_exabis_eportfolio_get_user_preferences($userid = null)
{
	...
	if (!$userpreferences || !$userpreferences->user_hash) {
		...
		block_exabis_eportfolio_set_user_preferences($userid, array('user_hash' => $hash, 'description'=>''));
